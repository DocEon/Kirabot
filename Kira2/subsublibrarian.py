...
def writeSearchResultsToFile(resultDictionary):
  ...
  for key in listOfKeys:
    fileName = re.search('.*\/beckett\/(.*)', key)
    if fileName == None:
      break
...
def processInput(text):
  ...
  if firstWord == 'hay':
    ...
    kirasearch(restOfText, chan)
  elif firstWord.lower() == 'beckettsearch':
      loadLogs()
      ...
      sendMsg("You can find your " + str(resultDictionary["Metadata"][1]) + " results at http://"+homeurl+"/beckett/results/"+restOfText+".html !", chan)
...
def buildMode(userDictionary):
  ...
  while command != "connect":
    ...
    elif command == "beckett":
      botnick = "subsublibrarian"
      logCopyDirectory = "/srv/http/beckett"
      channel = "#control"
      copyLogs = False
      logEnable = False
